...
<main class="container-lg py-3" style="margin-top: 75px; margin-bottom: 40px;">
    ...
    <div class="row">
        <h2 class="form-label">Edit User</h2>
    </div>

    <div class="row">
        <form method="POST" action="${appURL}/smp-edit-user" enctype="multipart/form-data">
            ...
            <!-- Role -->
            <div class="input-group mb-4">
                <label for="roleID" class="input-group-text">Role</label>
                <select class="form-select <c:if test="${not empty results.roleError}">is-invalid</c:if>" name="roleID" id="roleID">
                    <c:forEach items="${roles}" var="role">
                        <option value="${role.getRoleID()}" <c:if test="${results.roleID eq role.getRoleID()}">selected</c:if> data-bs-toggle="tooltip" title="${role.getDescription()}">${role.getRoleID()}</option>
                    </c:forEach>
                </select>
                <c:if test="${not empty results.roleError}">
                    <div class="invalid-feedback">${results.roleError}</div>
                </c:if>
            </div>

            <div class="btn btn-group p-0">
                <a onclick="Confirm('${results.userID}')" class="btn btn-danger">Delete User</a>
